 ...
 */
static CGFloat LACGVectorDot(CGVector a, CGVector b) {
 ...
 */
static CGVector LACGVectorScaled(CGVector v, CGFloat scale) {
    return (CGVector) {
        .dx = v.dx * scale,
        .dy = v.dy * scale
    };
 ...
 */
static CGVector LACGVectorRotatedRadians(CGVector v, CGFloat radians) {
 ...
 */
static MKMapPoint LAMKMapPointMidpoint(MKMapPoint a, MKMapPoint b) {
    return (MKMapPoint) {
        .x = (a.x + b.x) * 0.5,
        .y = (a.y + b.y) * 0.5
    };
 ...
 */
static CGPoint LACGPointAverage(CGPoint a, CGPoint b) {
    return (CGPoint) {
        .x = (a.x + b.x) * 0.5,
        .y = (a.y + b.y) * 0.5
    };
 ...
 */
static CLLocationDistance LACLDistanceBetweenLocationCoordinates2D(CLLocationCoordinate2D a, CLLocationCoordinate2D b) {
...
@implementation LACoordinateConverter

- (instancetype)initWithAnchorPair:(LAGeoAnchorPair)anchors {
    self = [super init];
    if (self) {
        _anchors = anchors;
         ...
         */
        CGVector cgDueEast = LACGVectorRotatedRadians(pdfDisplacement, radiansClockwiseOfDueEast);
        
        // Now, get the distance (in meters) between the two anchors...
        CLLocationDistance distanceBetweenAnchorsMeters = LACLDistanceBetweenLocationCoordinates2D(anchors.fromAnchor.latitudeLongitude, anchors.toAnchor.latitudeLongitude);
        
        // ...and rescale so that it's exactly one meter in length.
        _oneMeterEastward = LACGVectorScaled(cgDueEast, 1.0 / distanceBetweenAnchorsMeters);
         ...
         */
        _oneMeterSouthward = LACGVectorRotatedRadians(_oneMeterEastward, -M_PI_2);
         ...
         */
        MKMapPoint tangentMercatorCoordinate = LAMKMapPointMidpoint(fromAnchorMercatorCoordinate, toAnchorMercatorCoordinate);
        
        _tangentLatitudeLongitude = MKCoordinateForMapPoint(tangentMercatorCoordinate);
        
        _tangentPDFPoint = LACGPointAverage(anchors.fromAnchor.PDFPoint, anchors.toAnchor.PDFPoint);
...
- (MKMapPoint)MKMapPointFromPDFPoint:(CGPoint)PDFPoint {
    ...
    CGFloat dotProductEast = LACGVectorDot(displacementFromTangentPoint, _oneMeterEastward);
    CGFloat dotProductSouth = LACGVectorDot(displacementFromTangentPoint, _oneMeterSouthward);
    
    AAPLEastSouthDistance eastSouthDistanceMeters = {
        // How many meters Eastward is cgPoint from tangentPoint?
        .east = dotProductEast / LACGVectorDot(_oneMeterEastward, _oneMeterEastward),
        
        // How many meters Southward is cgPoint from tangentPoint?
        .south = dotProductSouth / LACGVectorDot(_oneMeterSouthward, _oneMeterSouthward)
    };
...
- (CGAffineTransform)PDFToMapKitAffineTransform {
    ...
    CGAffineTransformScale(
                           ...
                           1.0 / LACGVectorDot(_oneMeterEastward, _oneMeterEastward),
                           1.0 / LACGVectorDot(_oneMeterSouthward, _oneMeterSouthward)
                           );
